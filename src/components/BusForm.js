...
function BusForm( {
    ...
    const onFinish = async(values)=>{
        try{
            ...
            const { name, number, capacity, from, to, journeyDate, departure, arrival, fare } = values;

        // Convert required fields to correct types
        const formattedValues = {
            ...
            from,
            to,
            journeyDate: moment(journeyDate).format("YYYY-MM-DD"), // Ensure correct date format
            departure,
            arrival,
            fare: Number(fare),  // Convert string to number
            userId: "67e3cfd5155b3813e35c1e80", // Ensure userId is included
            // status: busstatus || "Yet To Start",  // ✅ Add default value
        };
