...
function AdminBuses() {
  ...
  const getBuses = async() =>{
    try{
      ...
      const response = await axiosInstance.post(`${process.env.REACT_APP_API_URL}/buses/get-all-buses`, {});
  ...
  const deleteBus = async(id) =>{
    try{
      dispatch(ShowLoading());
      const response = await axiosInstance.post(`${process.env.REACT_APP_API_URL}/buses/delete-bus`, {
        _id: id,
      });
      dispatch(HideLoading());
      if(response.data.success){
